...
echo "<table border=\"1\"  >";
     //   echo "<caption>  </caption>";
        echo " <thead><th>nom hotel</th><th>numchambre</th><th>prixnuit</th><th>typechmbre</th><th>typechmbre</th></thead>";
        
 
 while ($row= mysqli_fetch_assoc($query)) {//1
    //`numhotel``ville``nom``etoiles`
    $numhotel=$row['numhotel']; 
   
$query2=  mysqli_query($con," SELECT COUNT(chambre.typechmbre)as nbreresarvasion ,chambre.typechmbre,chambre.numhotel FROM `hotel` JOIN chambre join reservation 
 WHERE (chambre.`numhotel`=hotel.numhotel AND chambre.numchambre=reservation.numchambre and chambre.numhotel=reservation.numhotel and reservation.numhotel=$numhotel)
 GROUP BY chambre.numhotel,chambre.typechmbre ");
        $max=0;
                   while ($row2= mysqli_fetch_assoc($query2)) {//1
                    //`numhotel``ville``nom``etoiles`
                    $nb=$row2['nbreresarvasion']; 
           ...
           $query3=  mysqli_query($con,"SELECT  `numhotel`, `nom`, `ville`, `etoiles` FROM `hotel` WHERE `numhotel`=$numhotel");      
              $row3= mysqli_fetch_assoc($query3);
               $nom=$row3['nom'];    
                $ville=$row3['ville'];    
                 $etoiles=$row3['etoiles'];    
                 
              
                                                         echo "<tr><td>$numhotel </td><td>$nom  </td> <td>$ville</td><td>$etoiles</td><td>$tp</td><td>"
."</tr>";             
